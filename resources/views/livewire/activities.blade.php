<div>
            ...
            <div class="mt-2 ml-2">
                @can('assignment-create')
                    <flux:modal.trigger name="create-activity">
                        <flux:button icon:trailing="plus">Create Activity</flux:button>
                    </flux:modal.trigger>
                @endcan
            </div>
            <div class="mt-2 ml-2">
                @can('assignment-create')
                    <flux:button  variant="primary" icon:trailing="eye">View submissions</flux:button>
                @endcan
            </div>
        </div>
    </div>

    {{-- Create Acitivity Modal Definition --}}
    <flux:modal name="create-activity" class="md:w-150">
        <form wire:submit.prevent="submit">
            <div class="space-y-6">
                <div>
                    <flux:heading size="lg">Create Acitivity</flux:heading>
                    <flux:text class="mt-2">Enter the activity details below.</flux:text>
                </div>
                <div>
                    <flux:label for="newActivityCourse" class="block text-sm font-medium text-gray-700 mb-1">Course</flux:label>
                    <flux:select wire:model="newActivityCourseId" id="newActivityCourse">
                ...
                <div>
                    <flux:input label="Title" id="activityTitle" wire:model="title" placeholder="e.g., Chapter 5 Activity"/>
                </div>
                <div>
                    <flux:textarea label="Description / Instructions" id="activityBody" wire:model="body" placeholder="Enter activity instructions..." rows="5"/>
                </div>
                <div>
                ...
                <div class="flex justify-end space-x-3 pt-4 border-t">
                    ...
                    <flux:button type="submit" variant="primary">
                        <span wire:loading.remove wire:target="submit">Create Activity</span>
    ...
    <flux:modal name="upload-activity" class="md:w-150">
        <div class="space-y-6">
            <div>
                <flux:heading size="lg">Upload Activity</flux:heading>
                <flux:text class="mt-2">Upload your activity below.</flux:text>
            </div>
            <div>
                <flux:input label="Title of upload" wire:model="title" placeholder="e.g., Chapter 5 Activity"/>
            </div>
            <div>
                <flux:textarea label="Description of upload" wire:model="body" placeholder="Enter activity description..." rows="5"/>
            </div>
            <div>
            ...
            <div class="flex justify-end space-x-3 pt-4 border-t">
                ...
                <flux:button type="submit" wire:click="uploadActivity" variant="primary">
                    Upload
                </flux:button>
            </div>
        </div>
            ...
            @foreach($activities as $activity)
                <div class="w-full sm:w-4/5 lg:w-275  mx-auto p-4 bg-white border border-black rounded-lg shadow-md mb-4 flex flex-col justify-end items-end">
                    ...
                    <div class="w-full pb-4">
                        <h2 class="text-lg font-semibold text-gray-900 dark:text-white mb-2">{{ $activity->title }}</h2>
                        <p class="text-gray-600 dark:text-gray-300 mb-2">
                            {!! nl2br(e($activity->body)) !!}
                        </p>
                        <p class="text-sm text-gray-500 dark:text-gray-400">
                            Posted by: {{ $activity->creator->name }}
                        </p>
                        <p class="text-sm text-gray-500 dark:text-gray-400">
                            Course: {{ $activity->course->course_name }}
                        </p>
                        <p class="text-sm text-gray-500 dark:text-gray-400">
                    ...
                    <div class="w-full flex justify-between items-center border-t dark:border-gray-600 pt-3 {{ $activity->attachments->isEmpty() ? 'mt-3' : '' }}">
                        ...
                        <div class="flex-shrink-0 position-bottom">
                            <flux:button icon="upload" size="sm" variant="primary" wire:click="uploadshow({{ $activity->id }})">
                                <span class="hidden sm:inline">Upload Activity</span>
