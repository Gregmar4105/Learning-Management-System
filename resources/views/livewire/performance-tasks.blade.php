<div>
        ...
        <div class="flex">
            ...
            <div class="mt-2 ml-2">
                @can('assignment-create')
                    <flux:modal.trigger name="create-performance-task">
                        <flux:button icon:trailing="plus">Create Performance Task</flux:button>
                    </flux:modal.trigger>
                @endcan
            </div>
            <div class="mt-2 ml-2">
                @can('assignment-create')
                    <flux:button  :href="route('performance-task-upload')" variant="primary" icon:trailing="eye">View submissions</flux:button>
                @endcan
            </div>
        </div>
    </div>
    ...
    <flux:modal name="create-performance-task" class="md:w-150">
        <form wire:submit.prevent="submit">
            <div class="space-y-6">
                <div>
                    <flux:heading size="lg">Create Performance Task</flux:heading>
                    <flux:text class="mt-2">Enter the performance task details below.</flux:text>
                </div>
                <div>
                    <flux:label for="newPerformanceTaskCourse" class="block text-sm font-medium text-gray-700 mb-1">Course</flux:label>
                    <flux:select wire:model="newPerformanceTaskCourseId" id="newPerformanceTaskCourse">
                ...
                <div>
                    <flux:input label="Title" id="performanceTaskTitle" wire:model="title" placeholder="e.g., Chapter 5 Performance Task"/>
                </div>
                <div>
                    <flux:textarea label="Description / Instructions" id="performanceTaskBody" wire:model="body" placeholder="Enter performance task instructions..." rows="5"/>
                </div>
                <div>
                ...
                <div class="flex justify-end space-x-3 pt-4 border-t">
                    ...
                    <flux:button type="submit" variant="primary">
                        <span wire:loading.remove wire:target="submit">Create Performance Task</span>
    ...
    <flux:modal name="upload-performance-task" class="md:w-150">
        <div class="space-y-6">
            <div>
                <flux:heading size="lg">Upload Performance Task</flux:heading>
                <flux:text class="mt-2">Upload your performance task below.</flux:text>
            </div>
            <div>
                <flux:input label="Title of upload" wire:model="title" placeholder="e.g., Chapter 5 Performance Task"/>
            </div>
            <div>
                <flux:textarea label="Description of upload" wire:model="body" placeholder="Enter performance task description..." rows="5"/>
            </div>
            <div>
            ...
            <div class="flex justify-end space-x-3 pt-4 border-t">
                ...
                <flux:button type="submit" wire:click="uploadPerformanceTask" variant="primary">
                    Upload
                </flux:button>
            </div>
        </div>
    ...
    {{-- Display Area for Performance Tasks --}}
    <div class="space-y-4 mt-6">
        @if($performancetasks->count() > 0)
            @foreach($performancetasks as $performancetask)
                <div class="w-full sm:w-4/5 lg:w-275  mx-auto p-4 bg-white border border-black rounded-lg shadow-md mb-4 flex flex-col justify-end items-end">
                    ...
                    <div class="w-full pb-4">
                        <h2 class="text-lg font-semibold text-gray-900 dark:text-white mb-2">{{ $performancetask->title }}</h2>
                        <p class="text-gray-600 dark:text-gray-300 mb-2">
                            {!! nl2br(e($performancetask->body)) !!}
                        </p>
                        <p class="text-sm text-gray-500 dark:text-gray-400">
                            Posted by: {{ $performancetask->creator->name }}
                        </p>
                        <p class="text-sm text-gray-500 dark:text-gray-400">
                            Course: {{ $performancetask->course->course_name }}
                        </p>
                        <p class="text-sm text-gray-500 dark:text-gray-400">
                            {{ $performancetask->created_at->format('M d, Y h:i a') }}
                        </p>
                    </div>
                    {{-- START: Combined Attachments and Actions Container --}}
                    <div class="w-full flex justify-between items-center border-t dark:border-gray-600 pt-3 {{ $performancetask->attachments->isEmpty() ? 'mt-3' : '' }}">
                        {{-- Left Side: Attachments List --}}
                        <div>
                        @if ($performancetask->attachments->isNotEmpty())
    <div class="space-y-1">
        <p class="text-xs font-medium text-gray-500 dark:text-gray-400 mb-1">Attachments:</p>
        @foreach($performancetask->attachments as $attachment) {{-- Corrected variable name --}}
            @php
...
@endif
                        ...
                        <div class="flex-shrink-0 position-bottom">
                            <flux:button icon="upload" size="sm" variant="primary" wire:click="uploadshow({{ $performancetask->id }})">
                                <span class="hidden sm:inline">Upload Performance Task</span>
        ...
        @else
            <div class="text-center text-gray-500 dark:text-gray-400 py-6">
                No performance tasks found matching your criteria.
            </div>
        @endif
    </div>
</div>